...
{
#ifdef _256
    display_init( RESOLUTION_256x240, DEPTH_16_BPP, 1, GAMMA_NONE, ANTIALIAS_RESAMPLE );
#else
    display_init( RESOLUTION_320x240, DEPTH_16_BPP, 1, GAMMA_NONE, ANTIALIAS_RESAMPLE );
#endif
    /* Disable anti-aliasing on PAL and PAL-M */
    ...
    rl_screen = (uint16_t*)__get_buffer(disp);
    memset(rl_screen, 0, (SCREEN_WIDTH*SCREEN_HEIGHT)*2);
    display_show(disp);
	
	#ifdef FPS_COUNTER
    new_timer(TIMER_TICKS(1000000), TF_CONTINUOUS, (void (*)(int))fps_timer);
...
#ifdef _256
#define Y_OFFSET (8 * SCREEN_WIDTH)
#else
#define Y_OFFSET (0)
#endif

void nglStartDisplay()
{
	/* Grab a render buffer */
	while( !(disp = display_lock()) )
	{
	}
	screen->bitmap = (COLOR*)__get_buffer( disp ) + Y_OFFSET;
	background->bitmap = (COLOR*)__get_buffer( disp ) + Y_OFFSET;
	rl_screen = (uint16_t*)__get_buffer( disp ) + Y_OFFSET;
}
...
void nglDisplay()
{
	display_show(disp);
		...
		snprintf(fps_, sizeof(fps_), "FPS %d\n", fps_get());
		graphics_draw_text(disp, 16, 16, fps_);
    #endif
	...
	if (ckeys.c[0].y > DEAD_ZONE || ckeys.c[0].up) pad_game[0] = 1;
	else if (ckeys.c[0].y < -DEAD_ZONE || ckeys.c[0].down) pad_game[0] = 2;

	if (ckeys.c[0].x < -DEAD_ZONE || ckeys.c[0].C_left || ckeys.c[0].left) pad_game[1] = 1;
	else if (ckeys.c[0].x > DEAD_ZONE || ckeys.c[0].C_right || ckeys.c[0].right) pad_game[1] = 2;
			
	if (ckeys.c[0].C_up) pad_game[2] = 1;
	else if (ckeys.c[0].C_down) pad_game[2] = 2;

	if( keys.c[0].A ) pad_game[8] = 1;
	if( keys.c[0].B ) pad_game[9] = 1;
	
	if( keys.c[0].Z ) pad_game[10] = 1;
	if( keys.c[0].start ) pad_game[11] = 1;
	
	if( keys.c[0].L ) pad_game[12] = 1;
	if( keys.c[0].R ) pad_game[12] = 2;
...
#define pixel(x,y,z,c) z_buffer[x + y*SCREEN_WIDTH] = z; rl_screen[x + y*SCREEN_WIDTH] = c;
...
{
	// 190 is a magic number for the blue sky.
	__n64_memset_ASM(rl_screen, 190, (SCREEN_WIDTH*SCREEN_HEIGHT)*2);
	__n64_memset_ASM(z_buffer, UINT16_MAX,(SCREEN_WIDTH*SCREEN_HEIGHT)*2);
